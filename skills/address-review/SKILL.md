---
name: address-review
description: Use when the user has finished reviewing your change in Cairn and left comments — they'll say something like "address the review", "I left comments", or "go through my cairn comments". Reads .cairn/comments.json, fixes change-requests, answers questions, marks each one handled, and refreshes the tour so the next review pass shows what you did.
---

# Addressing review comments

The reviewer left comments on your change in Cairn. They live in `.cairn/comments.json`,
...
1. **Read** `.cairn/comments.json`. Each comment has `id`, `step`, `file`, `line`, `type`
...
3. **Write back** to each comment you handled:
   ...
   Write the full array back to `.cairn/comments.json` (valid JSON).

4. **Refresh the tour.** If your edits moved code or added/removed logic, re-run the
   **review-tour** skill so `.cairn/review-tour.json` reflects the new state (line ranges will
   have shifted). At minimum, run `cairn check` and make sure every change is still covered.

5. **Hand back.** Tell the user: *"Addressed N comments — refresh `cairn serve` to see the
...
- Don't forget to refresh the tour / run `cairn check` after edits — stale line numbers make re-review confusing.
