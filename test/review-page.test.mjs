...
import { renderFullFile, renderStoryStepPanel, renderFilePanelContent } from '../dist/render.js';
...
const renderSrc = readFileSync(new URL('../src/render.ts', import.meta.url), 'utf8');
...
function fixtureRepo({ story = true } = {}) {
  ...
  writeFileSync(join(dir, 'src', 'order.ts'), 'export function place() {\n  return 1;\n}\n');
  writeFileSync(join(dir, 'src', 'settle.ts'), 'export function settle() {\n  return 0;\n}\n');
  git('add', '.');
  git('commit', '-qm', 'base');
  writeFileSync(join(dir, 'src', 'order.ts'), 'export function place() {\n  return 2;\n}\n');
  writeFileSync(join(dir, 'src', 'settle.ts'), 'export function settle() {\n  return 7;\n}\n');
      ...
      JSON.stringify({
        ...
        steps: [
          ...
          },
          {
            ...
            beats: [
              { text: 'The return value moves from one to two.', highlights: [[2, 2]] },
              { text: 'Nothing else in this function changes.', highlights: [[1, 1]] },
            ],
          },
          {
            id: 's2',
...
test('the payload is metadata-first: no step ships a rendered diff', async () => {
  ...
  const { server, base, route } = await boot(repo);
  try {
    const payload = shellPayload(await (await fetch(`${base}${route}/review?story=story.json`)).text());
    ...
    for (const marker of ['ds-diffbody', 'ds-row ds-row-add', 'ds-urow', 'data-comment-code', 'tk-k']) {
      assert.ok(!serialized.includes(marker), `payload must not carry rendered diff markup (${marker})`);
    }
    for (const step of payload.steps) {
      assert.ok(!('blocks' in step) && !('moves' in step) && !('focusGroups' in step));
    }
...
test('the payload carries the speech projections a stub needs to plan narration', async () => {
  ...
  try {
    ...
    assert.ok(primer.conceptSpeech, 'a concept stub can be spoken without loading it');
    ...
    for (const beat of beated.beats) {
      ...
      assert.ok(beat.destination.startsWith('src/order.ts'), 'the spoken destination names the file');
    }
    const plain = payload.steps.find((step) => step.kind !== 'concept' && !step.beats.length);
    assert.ok(plain.why.speech, 'a beat-less step still speaks its review note');
  } finally {
    server.close();
  }
});
...
test('the reading-position key is scoped per story, not per scope', async () => {
  ...
  try {
    ...
    assert.ok(payload.viewedScope.startsWith(repo), 'the reviewed-mark scope is repo + scope');
  } finally {
    server.close();
  }
});
...
test('only open comments travel, each with its server-computed anchor', async () => {
  ...
  writeFileSync(
    ...
    JSON.stringify([
      { id: 'open', file: 'src/order.ts', line: 2, side: 'right', type: 'question', body: 'Why two?', selectedText: '  return 2;', status: 'open', createdAt: '2026-01-01T00:00:00Z' },
      { id: 'done', file: 'src/order.ts', line: 2, type: 'nit', body: 'ok', status: 'resolved', createdAt: '2026-01-01T00:00:00Z' },
    ]),
  );
  const { server, base, route } = await boot(repo);
  try {
    const payload = shellPayload(await (await fetch(`${base}${route}/diff`)).text());
    assert.deepEqual(payload.comments.map((c) => c.id), ['open']);
...
test('hostile authored text cannot terminate the payload script element', async () => {
  ...
  writeFileSync(
    join(repo, '.diffstory', 'comments.json'),
    JSON.stringify([
      { id: 'c1', file: 'src/order.ts', line: 2, side: 'right', type: 'question', body: hostile, status: 'open', createdAt: '2026-01-01T00:00:00Z' },
    ]),
  );
  const { server, base, route } = await boot(repo);
  try {
    ...
    assert.equal(shellPayload(html).comments[0].body, hostile, 'and the value round-trips byte for byte');
  } finally {
    server.close();
  }
});
...
test('AT RISK 1: lazy step stubs still ship the speech cache', () => {
  ...
  const plan = engine.match(/function speechUnitsForStep\([\s\S]*?\n {2}\}/)
...
test('AT RISK 2: the reading position is keyed by scope AND story', () => {
  ...
  assert.match(key, /data-story-key/, 'commit 2156520: scope alone replays one story into another');
...
test('AT RISK 3: the measurement-only row attributes still exist end to end', () => {
  const renderTs = readFileSync(new URL('../src/render.ts', import.meta.url), 'utf8');
  // Emitted server-side...
...
test('AT RISK 4: the arrow keys are resolved by one handler in a fixed order', () => {
  const onKey = engine.match(/function onKey\(e\)\{[\s\S]*?\n {2}\}\n {2}function /)[0];
  const order = [
    'data-rail-beat',
    'data-story-beat',
    'moveSpeechBeat',
  ...
  assert.ok(order.every((at) => at >= 0), 'all five arrow-key claimants are in the one handler');
  assert.deepEqual([...order].sort((a, b) => a - b), order, 'rail beat → story beat → speech → panel beat → change nav');
...
test('all twenty keyboard bindings survive the move', () => {
  const bindings = [
    ...
    ["split divider arrows", /aria-keyshortcuts','ArrowLeft ArrowRight Home End/],
    ["row C hint", /aria-keyshortcuts/],
  ];
  for (const [name, pattern] of bindings) assert.match(engine, pattern, `keyboard binding: ${name}`);
  ...
  const changeShortcut = engine.match(/function handleChangeShortcut\(e\)\{[\s\S]*?\n {2}\}/)[0];
  for (const key of ["'n'", "'N'", "'p'", "'P'", "'\\['", "'\\]'"]) {
    assert.match(changeShortcut, new RegExp(key), `change navigation still binds ${key}`);
  }
});
...
test('AT RISK 5: every lazy endpoint distinguishes a stale lease from a retry', () => {
  ...
  assert.ok(reloads.length >= 4, `every inline error surface offers a reload (${reloads.length})`);
...
test('AT RISK 6: coverage is pending until it is checked, never assumed clean', () => {
  assert.match(reviewViewSrc, /data-trust-uncovered=\{trust\.pending \? "" : trust\.uncoveredCount\}/);
...
test('AT RISK 7: reviewed marks stay bound to the exact file diff', () => {
  ...
  assert.match(engine, /Array\.isArray\(stored\)/, 'legacy string[] entries migrate');
...
test('AT RISK 8: a selection may not cross diff sides', () => {
  ...
  const context = engine.match(/function currentSelectionContext\(\)\{[\s\S]*?\n {2}\}/)[0];
  assert.match(context, /return null/);
});

test('AT RISK 9: the Mermaid sanitizer keeps every one of its rules', () => {
  const sanitize = engine.match(/function sanitizeMermaidSvg\(svg\)\{[\s\S]*?\n {2}\}/)[0];
  ...
  assert.match(sanitize, /querySelectorAll\('script,foreignObject,iframe,object,embed,image,a'\)/);
...
test('AT RISK 13/14: live banners are per-generation and the drop has a grace period', () => {
  ...
  assert.match(engine, /liveDisconnectTimer=setTimeout\([\s\S]{0,120}?4000\)|4000\)/);
...
test('modal inerting reaches the modal\'s real siblings, not just body\'s children', () => {
  ...
  const sync = engine.match(/function syncModalBackground\(\)\{[\s\S]*?\n {2}\}/)[0];
  assert.match(sync, /modalBackgroundSnapshots\.forEach\(restoreModalNode\)/, 'restore before re-applying');
...
test('every performance measure that keeps a 300-step story usable is still here', () => {
  const measures = [
    ['single-step prefetch only', /loadStoryStep\(i\+1\)|loadStoryStep\(index\+1\)/],
    ...
    ['rAF-batched drags', /requestAnimationFrame\(applySplitResize|splitResizeFrame/],
  ...
  for (const [name, pattern] of measures) assert.match(engine, pattern, `performance measure: ${name}`);
...
test('the route still builds its model metadata-first', () => {
  const renderTs = readFileSync(new URL('../src/render.ts', import.meta.url), 'utf8');
  const call = renderTs.match(/buildReviewModel\(repo, tour, files, headRef, \{[\s\S]*?\}\);/)[0];
  assert.match(call, /detailedStepIndexes: input\.fileIndex \? new Set\(\) : new Set\(\[0\]\)/);
...
test('no beUI component with a baked-in live region reaches this surface', () => {
  ...
  const surface = [reviewApp, sidebar, storyView, reviewViewSrc, progressHost, entry].join('\n');
  assert.ok(!/vendor\/beui/.test(surface), 'this surface is hand-built; it imports no beUI component at all');
  assert.ok(!surface.includes('streaming'), 'nothing here passes a streaming prop');
...
test('the accessibility contract the old renderer guaranteed is still in the markup', () => {
  ...
  assert.match(engine, /'role','group'[\s\S]{0,80}'aria-label','Comment type'|aria-label','Comment type'/);
  ...
  assert.match(reviewApp, /<div className="ds-command-scrim" data-shortcuts-close aria-hidden="true" \/>/);
  assert.doesNotMatch(reviewApp, /<button className="ds-command-scrim"/);
  assert.match(reviewApp, /aria-label="Close commands"/);
  assert.match(reviewApp, /className="ds-command-list" role="group" aria-label="Review commands"/);
  // Landmarks and the three view tabpanels.
  assert.match(sidebar, /<aside className="ds-rail" aria-label="Review navigation">/);
  assert.match(reviewApp, /<main className="ds-main">/);
  for (const view of ['tour', 'files', 'review']) {
    assert.ok(
      markup.includes(`id="ds-view-${view}" role="tabpanel" aria-labelledby="ds-tab-${view}"`),
...
test('a lazily served step panel carries the step, focus and move attributes', () => {
  ...
  assert.match(panel, /data-split-inner data-loaded="1"/, 'story steps never fetch split');
  ...
  const unified = panel.slice(panel.indexOf('data-diff-inner'), panel.indexOf('data-split-inner'));
  ...
  assert.doesNotMatch(filePanel, /data-split-inner data-loaded/, 'All-files split is a second lazy fetch');
});
...
test('the built bundle actually ships the review behaviour', (t) => {
  ...
  const js = readdirSync(dir)
    .filter((file) => file === 'review.js' || (file.startsWith('chunk-') && file.endsWith('.js')))
  ...
  assert.ok(!js.includes('scrollIntoView'), 'the horizontal-scroll trap stays out of the bundle');
});
...
test('the engine is one module with one entry point and two seams', () => {
  ...
  assert.equal(imports.length, 1, 'exactly one import: the progress-panel seam');
