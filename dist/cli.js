...
function cmdCheck(a) {
    ...
    console.log(`steps: ${tour.steps.length} · changed files: ${cov.totalChangedFiles} · covered: ${cov.coveredChangedFiles}`);
    if (cov.uncovered.length) {
        console.log(`\n⚠ ${cov.uncovered.length} change(s) not in the tour:`);
        for (const u of cov.uncovered)
            console.log(`   ${u.file}:${u.range[0]}-${u.range[1]} (${u.status})`);
    }
    else {
    ...
    if (stale.length) {
        ...
        for (const s of stale)
            console.log(`   #${s.order} ${s.title} → ${s.file}:${s.range[0]}-${s.range[1]}`);
    }
    console.log('');
