...
function rowHtml(type, no, content, intra) {
    const sign = type === 'add' ? '+' : type === 'del' ? '−' : ' ';
    return (`<div class="dv-row dv-${type}">` +
...
function pickSide(map, row, type) {
...
function hunksBody(f) {
    if (!f.hunks.length)
        return `<div class="dv-note">No line changes (metadata only).</div>`;
    return f.hunks
        .map((h) => {
...
export function renderDiffFullBody(rows) {
    if (!rows.length)
        return `<div class="dv-note">Couldn't read the file from the working tree.</div>`;
...
function filePanel(f) {
    ...
    const toggle = canFull
        ? `<div class="dv-toggle" role="group" aria-label="View mode">` +
            ...
            `</div>`
        : '';
    return (`<section class="dv-file" data-file="${esc(f.newPath)}" data-loaded="0">` +
        `<div class="dv-fhead">` +
...
export function diffViewStyles() {
    return `
...
export function diffViewScript() {
    return `(function(){
