...
export function kokoroVoiceOptions() {
    return Object.keys(KOKORO_VOICES).map((id) => ({ id, ...KOKORO_VOICES[id] }));
}
export function kokoroVoiceLabel(voice) {
    return KOKORO_VOICES[normalizeKokoroVoice(voice)].label;
}
export function kokoroVoiceLangCode(voice) {
    return KOKORO_VOICES[normalizeKokoroVoice(voice)].langCode;
}
export function kokoroRate(rate = 1) {
    const scale = Number.isFinite(rate) && rate > 0 ? Math.max(0.6, Math.min(1.5, rate)) : 1;
    return Number(scale.toFixed(2));
}
export function normalizeKokoroText(text) {
    return String(text ?? '')
        .replace(/\r\n?/g, '\n')
        .split(/\n+/)
...
export function kokoroTtsCachePath(home, input) {
...
export function kokoroTtsCacheDir(home) {
    return join(home, '.diffstory', 'tts-cache', 'kokoro');
}
export function kokoroTtsVenvDir(home) {
    return join(home, '.diffstory', 'kokoro-venv');
}
export function kokoroPythonCommand(home, override) {
...
export function kokoroTtsUrl(id) {
    return `/api/tts/kokoro/${id}.wav`;
}
export function isKokoroTtsId(id) {
    return /^[a-f0-9]{64}$/.test(id);
}
export async function synthesizeWithKokoro(home, input, opts = {}) {
    const text = normalizeKokoroText(input.text);
    if (!text)
        throw new Error('No text to speak.');
    if (text.length > KOKORO_MAX_TEXT)
        throw new Error(`Text is too long for Kokoro speech (${KOKORO_MAX_TEXT} chars max).`);
    if (opts.signal?.aborted)
        throw speechCancelled();
    const entry = kokoroTtsCachePath(home, { ...input, text });
    if (existsSync(entry.path))
        return { ...entry, cached: true, url: kokoroTtsUrl(entry.id) };
...
function ensureKokoroHelper(dir) {
...
function runKokoro(command, args, text, outputPath, signal) {
