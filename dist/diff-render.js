...
export function wholeUntoured(rows) {
    let any = false;
...
export function untouredRuns(rows) {
    const runs = new Map();
    let head;
    for (const row of rows) {
        if (!untouredRow(row)) {
...
function singleCell(row, target, tag) {
...
export function renderSplitRow(row, opts = {}) {
    if (!opts.single)
        throw new Error('renderSplitRow renders single-cell rows only; use SplitColumns');
    const target = opts.rightTarget ?? opts.leftTarget;
    const attrs = reviewRowAttrs(target, row.type, row.rightContent ?? row.leftContent ?? row.content, target ? opts.stepId : undefined, row.changePair);
    return `<div class="ds-row ds-row-${row.type}${row.changePair ? ' ds-row-pair' : ''}"${attrs}${rowExtraAttrs(opts)}>${singleCell(row, opts.rightTarget, opts.untouredTag)}</div>`;
}
...
export function renderSplitHalves(row, opts = {}) {
...
export function splitDividerHtml() {
...
function gapButtons(gap, chunk, edge) {
    const up = gap.to === 'eof'
        ? ''
...
function gapDataAttrs(gap, chunk) {
...
export function renderHunkGap(gap, opts = {}) {
    if (!gap)
        return `<div class="ds-hunkgap"><span class="ds-gaplabel">Skipped lines</span></div>`;
