...
export function editorNavigationTarget(repo, file, line, column) {
    if (!file ||
        isAbsolute(file) ||
...
export function editorLabel(editor) {
...
export function zedLaunchArgs(target) {
...
export function openVSCodeTargetWithUrls(target, openExternal) {
    if (!openExternal(vscodeFolderUrl(target)))
        return false;
    return openExternal(vscodeFileUrl(target));
}
function editorCommands(editor, home) {
    if (editor === "zed") {
...
export function openEditorTarget(target, editor) {
