...
export function renderPage(input) {
    ...
    const approveReady = openCount === 0 && uncoveredCount === 0 && storyFreshness === 'current';
    // No story → no coverage to report, so the coverage row is meaningless; hide it.
    const trustPill = storyless
        ? ''
        : `<button class="ds-trustpill${storyFreshness === 'current' && !uncoveredCount ? ' is-clean' : ''}" data-trust-open title="Trust check — story freshness and changes the story does not explain">${storyFreshness !== 'current'
            ...
            : uncoveredCount
                ...
                : `<span class="ds-check">✓</span><span>${focusedStory ? 'Story matches this diff and covers its selected scope' : 'Story matches and covers this diff'}</span><span class="ds-review-row-arrow">›</span>`}</button>`;
...
<body${storyless ? ' data-storyless="1"' : ''} data-story-freshness="${storyFreshness}"${focusedStory ? ' data-story-scope="focused"' : ''} data-repo="${esc(repo)}" data-viewed-scope="${esc(`${repo}|${reviewState.scopeKey || baseLabel}|${reviewMode}`)}" data-review-scope="${esc(reviewState.scopeKey)}" data-review-round="${reviewState.round}" data-review-snapshot="${esc(reviewState.currentSnapshotId ?? '')}" data-current-review-mode="${reviewMode}"${reviewMode === 'since' ? ' data-initial-view="files"' : ''}>
<header class="ds-top">
  ...
  <div class="ds-actions">
    ...
    <div class="ds-review-menu-wrap">
      <button class="ds-review-menu${approveReady ? ' is-clean' : ''}" data-review-menu data-unexplained-count="${uncoveredCount}" data-story-freshness="${storyFreshness}" aria-haspopup="dialog" aria-expanded="false" aria-label="Review, ${openCount} unresolved ${plural(openCount, 'comment')}${storyFreshness !== 'current' ? ', story requires regeneration before approval' : uncoveredCount ? `, ${uncoveredCount} ${plural(uncoveredCount, 'change')} not explained by the story` : ''}" title="Open review status">
        <span class="ds-review-menu-dot" aria-hidden="true"></span>
        <span>Review</span>
      ...
      <div class="ds-review-menu-pop" data-review-menu-pop role="dialog" aria-label="Review status and actions" tabindex="-1" hidden>
        ...
        ? focusedStory
            ? 'This exact diff matches the story, its selected scope is covered, and there are no open comments'
            : 'This exact diff is covered and there are no open comments'
        ...
        ? focusedStory
            ? 'This exact diff matches; the selected story scope is covered and resolved.'
            : 'This exact diff is covered and resolved.'
        : openCount
    ...
    <div class="ds-readhead" data-rail="files" hidden>
      ...
      <div class="ds-filetools">
        ...
        <div class="ds-filefilters" role="group" aria-label="File filters">
          ...
          <button data-file-filter="seen" aria-pressed="false">Seen</button>
          <button data-file-filter="unseen" aria-pressed="false">Unseen</button>
...
function reviewSessionBar(input) {
    ...
    const resolveDetail = input.open
        ? `${input.open} open ${plural(input.open, 'comment')}`
        : input.addressed
            ? `${input.addressed} ${plural(input.addressed, 'reply')} to verify`
            : 'Clear';
    const decisionDetail = input.approveReady
    ...
    <div class="ds-reviewstatus-main">
      ...
      <span class="ds-reviewstatus-scope"><b>${input.files}</b> ${plural(input.files, 'file')} in scope</span>
    </div>
    <div class="ds-reviewstatus-signals">
      <span class="ds-statussignal${input.open ? ' is-attention' : input.addressed ? ' is-pending' : ' is-clear'}"><i aria-hidden="true"></i>${resolveDetail === 'Clear' ? 'Feedback clear' : resolveDetail}</span>
      <span class="ds-statussignal${input.approveReady ? ' is-ready' : ' is-attention'}"><i aria-hidden="true"></i>${decisionDetail}</span>
    </div>
    ${state.compareFrom ? `<div class="ds-roundmodes" role="group" aria-label="Review comparison" title="${changed ? `${changed} ${plural(changed, 'file')} changed since your feedback` : 'No code changes since your feedback'}">
      ...
      <button type="button" data-review-mode="since" class="${mode === 'since' ? 'is-active' : ''}"${changed ? '' : ' disabled'} aria-pressed="${mode === 'since'}">Since review</button>
    </div>` : ''}
  </div>`;
}
...
function introPanel(model, tour, freshness, routeBase) {
    ...
    const trustFact = freshness !== 'current'
        ...
        : trust
            ...
            : `<div class="ds-fact ds-fact-ok"><span class="ds-fact-n">✓</span><span class="ds-fact-l">${tour.storyScope?.excludedFiles?.length ? 'selected story scope explained' : 'every change explained'}</span></div>`;
    const freshnessNote = freshness === 'current'
        ? ''
        : `<div class="ds-freshness-callout" role="status"><span><b>${freshness === 'stale' ? 'The diff changed after this story was generated.' : 'This older story has no exact diff fingerprint.'}</b> Review the current diff without relying on coverage, or regenerate the story before approval.</span><a href="${esc(routeBase)}/change">Regenerate story</a></div>`;
    const start = first
...
export function commentHtml(c0) {
    ...
    const selection = c.selectedText
        ...
        : '';
    return `<div class="ds-comment status-${c.status}" data-comment-id="${esc(c.id)}" data-status="${c.status}" data-comment-file="${esc(c.file)}" data-comment-line="${c.line}" data-comment-step="${esc(c.step ?? '')}"${hasReply ? ' data-hasreply="1"' : ''}>
...
function filePanel(f, i, stepIndexById) {
    ...
    const toggle = f.hasFull
        ? `<div class="ds-modetoggle" role="group" aria-label="Diff display mode"><button data-mode="diff" aria-pressed="false">Unified</button><button data-mode="split" aria-pressed="false">Split</button><button class="is-active" data-mode="full" aria-pressed="true">Full file</button></div>`
        : f.hunks.length
...
function commentAnchorState(repo, headRef, c) {
...
function feedbackCard(repo, headRef, raw) {
    ...
    return `<article class="ds-feedback-card status-${c.status}" data-feedback-card data-feedback-status="${c.status}" data-feedback-anchor="${anchor}" data-comment-id="${esc(c.id)}" data-comment-file="${esc(c.file)}" data-comment-line="${c.line}" data-comment-step="${esc(c.step ?? '')}">
...
function feedbackDrawer(repo, headRef, comments, state) {
    ...
    const events = state.events.length
        ? state.events
            .map((event) => `<li class="ds-timeline-event"><span class="ds-timeline-dot kind-${event.kind}"></span><div><strong>${esc(event.label)}</strong>${event.detail ? `<span>${esc(event.detail)}</span>` : ''}<small>Round ${event.round} · ${esc(relativeTime(event.at))}</small></div></li>`)
            .join('')
