...
export function renderPage(input) {
    ...
    const reviewModeControls = reviewState.compareFrom
        ? `<div class="ds-roundmodes ds-review-menu-modes" role="group" aria-label="Review comparison" title="${reviewState.changedFiles.length
        ...
        <button type="button" data-review-mode="since" class="${reviewMode === 'since' ? 'is-active' : ''}"${reviewState.changedFiles.length ? '' : ' disabled'} aria-pressed="${reviewMode === 'since'}">Since review</button>
      </div>`
        : '';
    return `<!doctype html>
<html lang="en">
...
export function reviewTimelineEventsHtml(events) {
    if (!events.length)
        return '<li class="ds-drawer-empty">The timeline starts when this review is opened.</li>';
    return events
