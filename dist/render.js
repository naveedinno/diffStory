...
export function renderPage(input) {
    ...
    const approveReady = feedbackHealthy &&
        blockingOpenCount === 0 &&
        uncoveredCount === 0 &&
...
<body${storyless ? ' data-storyless="1"' : ''} data-story-freshness="${storyFreshness}" data-feedback-health="${feedbackHealthy ? 'healthy' : 'invalid'}"${focusedStory ? ' data-story-scope="focused"' : ''} data-repo="${esc(repo)}" data-viewed-scope="${esc(`${repo}|${reviewState.scopeKey || baseLabel}|${reviewMode}`)}" data-review-scope="${esc(reviewState.scopeKey)}" data-review-round="${reviewState.round}" data-feedback-version="${reviewState.feedbackVersion ?? 0}" data-blocking-feedback-digest="${esc(reviewState.blockingFeedbackDigest ?? '')}" data-review-snapshot="${esc(reviewState.currentSnapshotId ?? '')}" data-current-diff-hash="${esc(reviewState.currentDiffHash)}" data-review-page-token="${esc(input.reviewPageToken ?? '')}"${input.reviewFrom ? ` data-review-from="${esc(input.reviewFrom)}"` : ''} data-verdict-state="${verdictEvaluation?.state ?? 'none'}" data-verdict-decision="${currentVerdict?.decision ?? ''}" data-current-review-mode="${reviewMode}"${reviewMode === 'since' ? ' data-initial-view="files"' : ''}>
  ...
  <div class="ds-actions">
    ...
    <div class="ds-review-menu-wrap">
      ...
      <div class="ds-review-menu-pop" data-review-menu-pop role="dialog" aria-label="Review status and actions" tabindex="-1" hidden>
        ...
        <button class="ds-review-option ds-review-option-approve${currentApproved ? ' is-current' : ''}" data-verdict="approve"${approveReady && !currentApproved ? '' : ' disabled'} title="${approveReady
...
function verdictSummary(state) {
    ...
    if (!current)
        return '<div class="ds-verdict-state is-none">No decision saved for this diff.</div>';
    return `<div class="ds-verdict-state ${current.decision === 'approved' ? 'is-approved' : 'is-requested'}"><strong>${current.decision === 'approved' ? 'Approved' : 'Changes requested'}</strong><span>Bound to this exact diff · ${esc(relativeTime(current.createdAt))}</span>${current.note ? `<small>${esc(current.note)}</small>` : ''}</div>`;
}
...
function reviewMap(model, tour) {
    const codeSteps = model.steps
        .map((step, index) => ({ step, index }))
        .filter((item) => item.step.kind !== 'concept');
    const behaviors = codeSteps.slice(0, 4).map(({ step, index }) => `<button type="button" data-goto-step="${index + 1}"><span>${String(step.order).padStart(2, '0')}</span>${esc(step.title)}</button>`).join('');
...
function trustRailCard(trust) {
    ...
    <span class="ds-trustcard-body">
      ...
      <span class="ds-trustcard-sub">${n} ${plural(n, 'change')} in the rendered diff ${n === 1 ? "isn't" : "aren't"} explained by any step.</span>
    </span>
  </button>`;
}
...
function codeStepPanel(repo, s, i, total, comments) {
    ...
    const cues = reviewCues(s)
        .map((cue) => `<span class="ds-reviewcue${cue.source === 'authored' ? ' is-authored' : ''}" title="${cue.source === 'authored' ? 'Authored story tag' : 'Suggested from this step'}">${esc(cue.label)}</span>`)
        .join('');
...
export function renderFilePanelContent(f, stepIndexById) {
    const [dir, base] = splitPath(f.file);
...
function feedbackDrawer(repo, headRef, comments, state, model) {
