...
<div class="ds-layout">
  <aside class="ds-rail">
    ...
    <div class="ds-readhead" data-rail="tour">
      ...
      <div class="ds-readhead-track"><div class="ds-readhead-fill" id="ds-progress-fill" style="width:${model.totalSteps ? (100 / model.totalSteps).toFixed(2) : 0}%"></div></div>
    </div>
...
function railCard(s, i) {
    const flow = s.flow
        ? `<span class="ds-flowchip" title="Call flow — where this step leads next and returns to in the tour"><span class="ds-flowico">↳</span>${esc(s.flow)}</span>`
        : '';
    return `<button class="ds-stepcard${i === 0 ? ' is-active' : ''}" data-step-index="${i}" data-step-id="${esc(s.id)}">
    ...
    <span class="ds-stepcard-body">
      ...
      <span class="ds-stepcard-tags">
        <span class="ds-badge ds-badge-${s.kind === 'new-file' ? 'new' : s.kind}" title="What kind of file this is in the change">${esc(s.kindLabel)}</span>
        ${flow}
      </span>
    </span>
  </button>`;
...
function stepPanel(repo, s, i, total, comments) {
    ...
    return `<section class="ds-step" data-step-panel="${i}" data-step-id="${esc(s.id)}"${i === 0 ? '' : ' hidden'}>
...
function singleCell(row) {
    ...
    return `<span class="ds-cell ds-cell-single"><span class="ds-no">${no}</span><span class="ds-sign"></span><span class="ds-code">${esc(row.content) || ' '}</span></span>`;
}
