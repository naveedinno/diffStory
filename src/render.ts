...
export interface ReviewShellInput {
  repo: string;
  tour: Tour;
  files: DiffFile[];
  ...
  /**
   * Identity of the story on screen, stable while its steps are. Several
   * stories — and every regeneration of one — share a single base..head scope,
   * so the scope key alone cannot say whose saved reading position a page is
   * allowed to resume.
   */
  storyKey?: string;
...
function beatViews(step: CodeStepView): ReviewBeatView[] {
 ...
 */
function stepView(step: StepView): ReviewStepView {
...
function fileRow(file: FileView): ReviewFileRow {
  return {
    file: file.file,
    add: file.add,
    del: file.del,
