...
export function renderPage(input: RenderInput): string {
  ...
  const trustPill = showTrustPill
    ? `<button class="ds-trustpill${trustPillClean ? ' is-clean' : ''}${pillState === 'pending' ? ' is-unknown' : ''}${excludedFiles.length || indexDivergentFiles.length ? ' has-exclusions' : ''}" data-goto-review="evidence" data-trust-excluded="${excludedFiles.length}"${focusedStory ? ' data-trust-focused="1"' : ''} title="Trust check — story freshness, coverage, staged state, and files outside the bounded renderer">${
        pillState === 'divergent'
