...
const isWs = (s: string): boolean => /^\s*$/.test(s);

/** Highlighted HTML for each side of a paired change, with changed tokens marked.
 *  A side is present only where it applies (left = removed line, right = added). */
export interface IntraSides {
  left?: string;
  right?: string;
}
 ...
 */
export function diffLineTokens(oldLine: string, newLine: string): { left: string; right: string } | null {
  ...
  let commonReal = 0;
  for (let k = 0; k < n; k++) if (aCommon[k] && !isWs(a[k].text)) commonReal++;
  if (commonReal / denom < SIMILARITY_THRESHOLD) return null;
 ...
 */
export function pairChanges<T>(rows: T[], getType: (row: T) => string): Array<[number, number]> {
  ...
  while (i < rows.length) {
    ...
    const delStart = i;
    while (i < rows.length && getType(rows[i]) === 'del') i++;
    const addStart = i;
    while (i < rows.length && getType(rows[i]) === 'add') i++;
    const count = Math.min(addStart - delStart, i - addStart);
    for (let k = 0; k < count; k++) pairs.push([delStart + k, addStart + k]);
  }
  return pairs;
}
 ...
 */
export function intraLineMap<T>(
  rows: T[],
  getType: (row: T) => string,
  getContent: (row: T) => string,
): Map<T, IntraSides> {
  const map = new Map<T, IntraSides>();
