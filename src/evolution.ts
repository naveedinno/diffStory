...
import type { StoryArc, StoryEvolution, Tour } from './types.js';
...
export interface EvolutionVerification {
  ...
  displayable: boolean;
  commitCount: number;
  phaseCommitCounts: number[];
}

const CACHE_LIMIT = 128;
const verificationCache = new Map<string, EvolutionVerification>();

function resolveBoundary(
  value: string,
  manifest: CommitEvolutionManifest,
): { sha?: string; error?: string } {
  const matches = manifest.commits.filter((commit) =>
    commit.sha.toLowerCase().startsWith(value.toLowerCase()),
  );
  if (matches.length === 1) return { sha: matches[0].sha };
  if (matches.length === 0) return { error: `commit boundary "${value}" is outside the frozen first-parent history` };
...
export function normalizeEvolutionObject(
  ...
  const evolution = raw as Record<string, unknown>;
  evolution.baseSha = manifest.baseSha;
  evolution.headSha = manifest.headSha;
  if (!Array.isArray(evolution.phases)) return;
  for (const rawPhase of evolution.phases) {
    if (typeof rawPhase !== 'object' || rawPhase === null || Array.isArray(rawPhase)) continue;
    const phase = rawPhase as Record<string, unknown>;
    for (const field of ['firstCommit', 'lastCommit'] as const) {
      if (typeof phase[field] !== 'string' || phase[field].length >= 40) continue;
      const resolved = resolveBoundary(phase[field], manifest);
      if (resolved.sha) phase[field] = resolved.sha;
    }
  }
}
...
export function verifyEvolution(
  repo: string,
  tour: Tour,
  frozenManifest?: CommitEvolutionManifest,
  ...
  if (
    manifest.baseSha !== evolution.baseSha ||
  ...
  let previousStart = -1;
  let previousEnd = -1;
  const phaseCommitCounts: number[] = [];
  evolution.phases.forEach((phase, phaseIndex) => {
    ...
    if (first.error) errors.push(`evolution.phases[${phaseIndex}].firstCommit ${first.error}`);
    if (last.error) errors.push(`evolution.phases[${phaseIndex}].lastCommit ${last.error}`);
...
export function cachedEvolutionVerification(
  repo: string,
  storyIdentity: string,
  tour: Tour,
): EvolutionVerification {
  if (!tour.evolution) return { errors: [], warnings: [], displayable: false, commitCount: 0, phaseCommitCounts: [] };
...
export function evolutionPreservationErrors(
  tour: Tour,
  originalArc?: StoryArc,
  originalEvolution?: StoryEvolution,
): string[] {
  const errors: string[] = [];
  if (originalArc === undefined) {
    if (tour.storyArc !== undefined) errors.push('targeted repair must not introduce storyArc');
  ...
  if (originalEvolution === undefined) {
    if (tour.evolution !== undefined) errors.push('targeted repair must not introduce evolution');
    return errors;
  }
  if (!tour.evolution) {
  ...
  const protectedEvolution = (value: StoryEvolution) => ({
  ...
  if (!isDeepStrictEqual(
    protectedEvolution(tour.evolution),
    protectedEvolution(originalEvolution),
  )) {
...
export function clearEvolutionVerificationCache(): void {
  verificationCache.clear();
}

export function evolutionVerificationCacheSize(): number {
