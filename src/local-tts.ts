...
export type SayVoice = 'samantha' | 'daniel';

export function normalizeSayVoice(voice?: string): SayVoice {
  const v = String(voice ?? '').toLowerCase().trim();
  if (v === 'daniel' || v === 'danield') return 'daniel';
  return 'samantha';
}

export function sayVoiceName(voice?: string): 'Samantha' | 'Daniel' {
  return normalizeSayVoice(voice) === 'daniel' ? 'Daniel' : 'Samantha';
}

export function normalizeSayPreset(preset?: string): 'story' | 'flirty' | 'bass' | 'system' {
  const p = String(preset ?? '').toLowerCase();
  if (p === 'warm' || p === 'flirty') return 'flirty';
  if (p === 'precise' || p === 'reviewer' || p === 'deep' || p === 'bass') return 'bass';
  if (p === 'system') return 'system';
  return 'story';
}

export function sayVoiceForPreset(preset?: string): string {
  const p = normalizeSayPreset(preset);
  if (p === 'bass') return 'Daniel';
  return 'Samantha';
}

export function sayVoiceForRequest(input: Pick<LocalTtsRequest, 'voice' | 'preset'>): SayVoice {
  if (input.voice) return normalizeSayVoice(input.voice);
  return normalizeSayPreset(input.preset) === 'bass' ? 'daniel' : 'samantha';
}

export function sayRateForVoice(voice?: string, rate = 1): number {
...
export function sayRateForPreset(preset?: string, rate = 1): number {
  const p = normalizeSayPreset(preset);
  return sayRateForVoice(p === 'bass' ? 'daniel' : 'samantha', rate);
}

export function normalizeSayText(text: string): string {
  return String(text ?? '').replace(/\s+/g, ' ').trim();
}

export function localTtsCachePath(home: string, input: LocalTtsRequest): LocalTtsCacheEntry {
...
export function localTtsCacheDir(home: string): string {
  return join(home, '.diffstory', 'tts-cache', 'say');
}

export function localTtsUrl(id: string): string {
  return `/api/tts/say/${id}.m4a`;
}

export function isLocalTtsId(id: string): boolean {
  return /^[a-f0-9]{64}$/.test(id);
}

export async function synthesizeWithSay(
  home: string,
  input: LocalTtsRequest,
  opts: { command?: string; signal?: AbortSignal } = {},
): Promise<LocalTtsCacheEntry & { cached: boolean; url: string }> {
  const text = normalizeSayText(input.text);
  if (!text) throw new Error('No text to speak.');
  if (text.length > SAY_MAX_TEXT) throw new Error(`Text is too long for local speech (${SAY_MAX_TEXT} chars max).`);
  if (opts.signal?.aborted) throw speechCancelled();

  const entry = localTtsCachePath(home, { ...input, text });
  if (existsSync(entry.path)) return { ...entry, cached: true, url: localTtsUrl(entry.id) };
...
function runSay(command: string, args: string[], text: string, outputPath: string, signal?: AbortSignal): Promise<void> {
