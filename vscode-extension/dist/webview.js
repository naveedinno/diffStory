...
exports.renderDiffStoryWebview = renderDiffStoryWebview;
const path = __importStar(require("node:path"));
const model_1 = require("./model");
function renderDiffStoryWebview(model) {
...
function renderRepositories(model) {
...
function renderHistory(model) {
    if (!model.repo)
        return `<section class="destination-page">${renderDestinationBack('Review history', 'Choose a repository before opening its review history.')}<button class="primary-button" data-open-screen="repositories">Choose repository</button></section>`;
...
function renderDestinationBack(title, detail, heading = title) {
...
function renderHistoryEntry(entry, active) {
...
function renderComparison(model) {
    if (!model.repo)
        return `<section class="destination-page">${renderDestinationBack('Comparison setup', 'Choose a repository before deciding what code to compare.', 'What do you want to review?')}<button class="primary-button" data-open-screen="repositories">Choose repository</button></section>`;
...
function comparisonPreset(id, title, badge, from, to, detail) {
...
function comparisonLabel(base, head) {
...
function modeTab(mode, label, count, initial, attention = false) {
...
function renderReviewDecision(model) {
    ...
    const feedbackHealth = model.review.feedbackHealth ?? { status: 'healthy', source: 'missing' };
...
function workflowStep(order, title, hint, active) {
...
function renderFileList(model, seen) {
...
function renderFileRow(file, reviewed, comments) {
...
function renderGuideTeaser(model) {
    if (model.story) {
...
function renderGuideMap(story, guideCurrent) {
    ...
    const concepts = story.steps.filter((step) => !(0, model_1.isCodeStep)(step)).length;
...
function renderStopCopy(step, story) {
    ...
    const prepares = step.preparesFor.map((id) => story.steps.find((candidate) => candidate.id === id)).filter(Boolean);
...
function renderFlowLinks(step, story) {
    const outgoing = (step.calls ?? []).map((id) => story.steps.find((candidate) => candidate.id === id)).filter(Boolean);
    const returnsTo = step.returnsTo ? story.steps.find((candidate) => candidate.id === step.returnsTo) : undefined;
    if (!outgoing.length && !returnsTo)
        return '';
...
function renderGuideStatus(model) {
    const status = model.guideStatus ?? { state: 'unverified', activeScopeLabel: model.scopeLabel, canSwitchScope: false };
...
function renderFeedbackAction(model, counts) {
    ...
    if (counts.open) {
        if (!model.agents.length)
            return renderAgentSetup('feedback');
...
function renderAgentProgress(model) {
...
function renderAgentSetup(mode) {
...
function renderFeedbackCard(comment) {
...
function renderActivity(model) {
    ...
    if (!lines.length && events.length <= 1)
        return '';
...
function statusDetails(status) {
    if (status.startsWith('A'))
        return { short: 'A', label: 'Added', tone: 'added' };
    if (status.startsWith('D'))
        return { short: 'D', label: 'Deleted', tone: 'deleted' };
    if (status.startsWith('R'))
        return { short: 'R', label: 'Renamed', tone: 'renamed' };
    if (status === '?')
        return { short: 'U', label: 'Untracked', tone: 'untracked' };
...
function webviewScript(initialMode) {
    return `
