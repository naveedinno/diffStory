...
exports.computeStoryClaimCoverage = computeStoryClaimCoverage;
 ...
 */
function computeStoryClaimCoverage(tour, unifiedDiff) {
    const claims = new Map();
    for (const step of tour.steps) {
        if (!(0, model_1.isCodeStep)(step) || step.kind === 'context')
            continue;
...
function parseChangedRanges(raw) {
    const files = [];
    let file;
    let hunkRanges = [];
    let rangeStart;
    ...
    const flushRange = () => {
        if (rangeStart !== undefined && rangeEnd !== undefined)
            hunkRanges.push([rangeStart, rangeEnd]);
    ...
    const flushHunk = () => {
        ...
        flushRange();
        if (!hunkHadAddition)
            hunkRanges.push([hunkNewStart, hunkNewStart]);
    ...
    for (const line of raw.split('\n')) {
        ...
        if (line.startsWith('--- ')) {
            ...
            if (value === '/dev/null')
                ...
                file.oldPath = stripDiffPath(value);
            continue;
        }
        if (line.startsWith('+++ ')) {
            const value = line.slice(4);
            if (value === '/dev/null')
                ...
                file.newPath = stripDiffPath(value);
            continue;
        }
