// Vendored from starc007/ui-components — components/motion/theme-toggle.tsx (MIT)
import { Moon, Sun } from "lucide-react";
// Upstream: `next-themes` (README modification 2). Same `{ theme,
// resolvedTheme, systemTheme, setTheme }` surface, backed by diffStory's own
// `data-theme` plumbing.
...
export function useThemeToggle({
  ...
  // README modification 5: upstream gates the icon behind a `mounted` effect to
  // dodge next-themes' hydration mismatch. `src/theme.ts`'s pre-paint bootstrap
  // has already written `data-theme` by the time this first renders, so the
  // first paint is correct and the flag is a constant. Kept in the return shape
  // so callers of `useThemeToggle()` still typecheck against upstream.
  const mounted = true;
  useEffect(() => {
